...
export function LoginClient({ openAccess }: { openAccess: boolean }) {
  return (
    <WalletProviders>
      <AuthCard openAccess={openAccess} />
    </WalletProviders>
  );
}

function AuthCard({ openAccess }: { openAccess: boolean }) {
  ...
  // Open-access sign-up: address + password only — no whitelist, no wallet signature.
  async function onSignup(e: FormEvent) {
    e.preventDefault();
    setBusy(true);
    setError(null);
    const res = await authApi.register({ address: address.trim(), password });
    setBusy(false);
    if (res.ok) {
      setAuthed(true);
      router.replace('/');
  ...
  // Register (secure) / reset: prove ownership of the connected wallet, then set the password.
  ...
  return (
    <main className="grid min-h-dvh place-items-center px-6">
      <div className="w-full max-w-sm rounded-2xl border border-border bg-surface p-6 shadow-pop sm:p-8">
        ...
        {credentialsForm ? (
          <form onSubmit={mode === 'signin' ? onSignin : onSignup}>
            ...
            <Input
              ...
              placeholder={mode === 'signin' ? 'Password' : 'Password (min 8)'}
              aria-invalid={error !== null}
            ...
            <Button
              ...
              disabled={
                busy ||
                !ADDRESS_RE.test(address.trim()) ||
                (mode === 'signin' ? password.length === 0 : password.length < 8)
              }
        ...
        <div className="mt-4 flex flex-col items-center gap-1.5 text-xs">
          {mode === 'signin' && (
            <>
              ...
              >
                {openAccess ? 'No account? Create one' : 'No account? Create one with your wallet'}
              </button>
              <button
                type="button"
