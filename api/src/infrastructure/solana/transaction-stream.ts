...
export class TransactionStream {
  ...
  private connect(): void {
    ...
    t.onError((err) => this.logger.debug({ err }, 'transaction-stream: socket error (close drives reconnect)'));
  }
    ...
    this.chain = this.chain
      .then(async () => {
        ...
        if (outageMs > this.cfg.replayWindowMs) {
          this.logger.warn({ outageMs }, 'transaction-stream: outage exceeded replay window — RPC backfill');
  ...
  private onMessage(raw: string): void {
    ...
    this.chain = this.chain.then(() => this.handleNotification(parsed)).catch((err) => {
  ...
  private async advanceCursor(wallet: string, signature: string, slot: number): Promise<void> {
    ...
    try {
      ...
      this.logger.warn({ err, wallet }, 'transaction-stream: cursor persist failed (will retry next tx)');
    }
  }
  ...
  private fireGapBackfill(wallet: string): void {
    this.logger.warn({ wallet, tipSlot: this.observedTipSlot }, 'transaction-stream: suspected gap — backfill');
