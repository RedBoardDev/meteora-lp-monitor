...
import { CONFIG_DEFAULTS, type CopybotRuntimeConfig, CopybotConfigSchema, isValidConfigBlob, parseConfig } from '@/domain/copybot/config';
...
export class ConfigStore {
  ...
  async load(): Promise<CopybotRuntimeConfig> {
  ...
  async save(cfg: CopybotRuntimeConfig): Promise<void> {
  ...
  async seedIfAbsent(): Promise<CopybotRuntimeConfig> {
    const raw = await this.readRaw();
